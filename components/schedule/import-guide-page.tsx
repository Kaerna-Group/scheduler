    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="relative border-b border-border/80 bg-background/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1360px] flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-7 lg:px-10 xl:py-5">
          <a href="#/" className="inline-flex h-10 items-center gap-2 rounded-full px-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> До розкладу</a>
          <Select value={selectedUser} onValueChange={(value) => value && selectUser(value)}>
            <SelectTrigger aria-label="Користувач для імпорту" className="h-10 min-w-[210px] rounded-full border-border bg-card/80 px-3.5 text-xs font-semibold text-foreground shadow-none xl:h-11 xl:min-w-[240px] xl:text-sm">
      ...
      <div className="relative mx-auto max-w-[1360px] px-4 pb-24 pt-8 sm:px-7 lg:px-10 xl:pt-10">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-accent">Єдиний спосіб редагування</p>
            <h1 className="mt-2 max-w-3xl text-4xl font-semibold tracking-[-0.055em] text-foreground sm:text-5xl">Імпорт розкладу</h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-muted-foreground">Встав готовий JSON або завантаж файл. Спочатку виконай безпечну перевірку, потім імпортуй.</p>
          </div>
          <div className="text-right text-xs leading-6 text-muted-foreground"><div>{schedule.semester.title} · {schedule.semester.weeksCount} тижнів</div><div>{source === 'remote' ? `Синхронізовано · revision ${schedule.revision}` : 'Показано локальні дані'}</div></div>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(330px,.75fr)] xl:gap-8">
          <section className="rounded-[26px] border border-border bg-card/80 p-4 shadow-[0_16px_55px_rgb(var(--theme-shadow-color)/6%)] sm:p-6 xl:p-7">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div><h2 className="text-xl font-semibold tracking-[-0.035em] text-foreground">JSON для імпорту</h2><p className="mt-1 text-xs text-muted-foreground">Користувач: {schedule.user.displayName}</p></div>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              <button onClick={() => setMode('merge')} className={`rounded-[15px] border p-3 text-left transition ${mode === 'merge' ? 'border-ring bg-warning-soft' : 'border-border bg-background'}`}><div className="text-xs font-bold text-foreground">Merge</div><div className="mt-1 text-[11px] leading-5 text-muted-foreground">Додати або оновити перелічені дисципліни, не прибираючи інші.</div></button>
              <button onClick={() => setMode('replace')} className={`rounded-[15px] border p-3 text-left transition ${mode === 'replace' ? 'border-ring bg-warning-soft' : 'border-border bg-background'}`}><div className="text-xs font-bold text-foreground">Replace my enrollments</div><div className="mt-1 text-[11px] leading-5 text-muted-foreground">Залишити користувачу лише дисципліни з цього JSON.</div></button>
            </div>

            <label className="mt-3 flex items-start gap-3 rounded-[14px] bg-secondary p-3 text-xs leading-5 text-muted-foreground"><input type="checkbox" checked={allowSharedUpdates} onChange={(event) => setAllowSharedUpdates(event.target.checked)} className="mt-1" /><span><strong className="text-foreground">Дозволити конфліктні спільні зміни.</strong> Нова група додається без цього прапорця. Увімкни його лише для точкової заміни правила, яке перетинається з уже збереженим.</span></label>

            <Textarea value={importText} onChange={(event) => setImportText(event.target.value)} spellCheck={false} className="mt-4 min-h-[420px] rounded-[17px] bg-background font-mono text-xs leading-relaxed" aria-label="JSON розкладу" />
          ...
          <aside className="space-y-5 lg:sticky lg:top-5 lg:self-start">
            <section className="rounded-[24px] bg-primary p-5 text-primary-foreground shadow-[0_18px_45px_rgb(var(--theme-shadow-color)/15%)]">
              <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary-foreground/50">Для ChatGPT / Claude / Gemini</p><h2 className="mt-2 text-xl font-semibold tracking-[-0.035em]">Готовий промпт для LLM</h2></div><Clipboard className="size-5 text-accent" /></div>
              <p className="mt-3 text-xs leading-6 text-primary-foreground/65">Скопіюй правила, встав у модель, а наступним повідомленням надішли скриншот або текст розкладу.</p>
              <Button onClick={copyPrompt} className="mt-5 h-11 w-full rounded-[14px] bg-card text-foreground hover:bg-card/90">{copied ? <Check className="size-4" /> : <Clipboard className="size-4" />}{copied ? 'Промпт скопійовано' : 'Скопіювати промпт'}</Button>
            </section>

            <section className="rounded-[24px] border border-border bg-card/75 p-5">
              <h2 className="text-sm font-bold text-foreground">Безпечний порядок</h2>
              <ol className="mt-4 space-y-3 text-xs leading-5 text-muted-foreground">{['Вибери правильного користувача.', 'Встав його персональний token.', 'Встав JSON та натисни «Перевірити».', 'Переглянь помилки або план змін.', 'Лише потім натисни «Імпортувати».'].map((step, index) => <li key={step} className="flex gap-3"><span className="grid size-6 shrink-0 place-items-center rounded-full bg-secondary text-[10px] font-bold text-foreground">{index + 1}</span><span>{step}</span></li>)}</ol>
            </section>

            <section className="rounded-[24px] border border-warning/35 bg-warning-soft p-5"><div className="flex gap-3"><ShieldAlert className="mt-0.5 size-5 shrink-0 text-warning" /><div><h2 className="text-sm font-bold text-warning-foreground">Спільні дані</h2><p className="mt-2 text-xs leading-6 text-warning-foreground">Назва та lessons дисципліни зі спільним externalCode використовуються всіма. Нові групи й непересічні правила додаються до offering, а відсутні в JSON правила не видаляються.</p></div></div></section>
            ...
            <details className="group rounded-[24px] border border-border bg-card/75 p-5">
              <summary className="flex cursor-pointer list-none items-center gap-3 text-sm font-bold text-foreground">
                ...
                <span className="ml-auto text-[10px] font-semibold text-muted-foreground group-open:hidden">Показати</span>
              </summary>
        ...
        <section className="mt-12 border-t border-border pt-10">
          <div className="max-w-3xl"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Повна специфікація</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.05em] text-foreground">Усі правила імпорту</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">Цей перелік відповідає перевіркам браузера та Apps Script. Його можна передавати іншій людині або LLM без доступу до коду проєкту.</p></div>
        ...
        <section className="mt-12 grid gap-6 lg:grid-cols-2">
          <div className="rounded-[24px] border border-border bg-card/75 p-5 sm:p-6">
            <h2 className="text-xl font-semibold tracking-[-0.035em] text-foreground">Точні допустимі значення</h2>
            <div className="mt-5 overflow-x-auto"><table className="w-full border-collapse text-left text-xs"><thead><tr className="border-b border-border text-muted-foreground"><th className="pb-3 pr-4 font-semibold">Поле</th><th className="pb-3 font-semibold">Значення</th></tr></thead><tbody className="align-top text-foreground">{[
