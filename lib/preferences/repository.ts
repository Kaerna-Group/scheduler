...
export function getStalePreferences(
  error: unknown,
  ...
  if (!details.preferences || !Number.isInteger(details.preferencesRevision))
    return null;
  return {
    preferences: details.preferences as SchedulerPreferences,
    preferencesRevision: Number(details.preferencesRevision),
  };
}
